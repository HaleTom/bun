...
// Windows MSVC lacks most POSIX file-type macros. The Stats bindings
// below need them, but we must NOT leak the fallbacks to sibling TUs in a
// unified-source bundle (since #29545 every `.cpp` under this directory
// is concatenated into one TU). ProcessBindingConstants.cpp and
// NodeConstantsModule.h check `#ifdef S_IFBLK` / `#ifdef S_IFSOCK` before
// exporting those to `fs.constants` — without this guard the Windows
// build spuriously exports them.
...
#if OS(WINDOWS)
// Mirror the push_macro block at the top: restore whatever the POSIX
// file-type macros were before this file (undefined on Windows). Without
// this a unified-source bundle would leak S_IFBLK / S_IFSOCK into the
// sibling cpps that gate `fs.constants` on `#ifdef`.
