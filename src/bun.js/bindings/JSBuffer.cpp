...
static JSC::EncodedJSValue jsBufferPrototypeFunction_fillBody(JSC::JSGlobalObject* lexicalGlobalObject, JSC::CallFrame* callFrame, typename IDLOperation<JSArrayBufferView>::ClassParameter castedThis)
{
    ...
    // Capture byteLength exactly once up front, used for the initial
    // validateNumber(end) range and as the default `end`. It's a fresh read
    // and not used for any memory access; the final write range is clamped
    // against a single re-read of byteLength after all JS coercions have
    // finished (see the check right before memset/memmove).
    ...
    // ── 1. Run every JS-visible coercion FIRST ──────────────────────────
    // Each of these can call user-supplied valueOf / toString which may
    // transfer() (detach) or resize() the backing ArrayBuffer. Gathering
    // all side effects here means the pointer + byteLength read later
    // sees the post-coercion state with a single read.

    // 1a. Encoding (only relevant for the string-value branch). toString
    //     on an object encoding is the first callback.
    if (!encodingValue.isUndefined() && value.isString()) {
        encoding = parseEncoding(scope, lexicalGlobalObject, encodingValue, true);
        RETURN_IF_EXCEPTION(scope, {});
    }

    // 1b. offset / end. validateNumber rejects non-numbers before any
    //     coercion, so no user JS runs here; these are pure reads.
    ...
    // Short-circuit empty ranges BEFORE value coercion. Matches Node.js:
    // `buf.fill(value, 5, 3)` returns `buf` without ever inspecting
    // `value`, so a throwing valueOf / empty Uint8Array / detached view
    // passed together with an empty range stays a no-op. The second check
    // further down clamps against post-coercion byteLength and handles
    // the case where valueOf itself shrinks/detaches castedThis.
    if (offset >= end) {
        RELEASE_AND_RETURN(scope, JSValue::encode(castedThis));
    }

    // 1c. Value coercion per branch. Each branch finishes ALL of its JS
    //     work before touching `castedThis`'s memory.
    ...
    if (value.isString()) {
        ...
        if (viewValue->byteLength() == 0) [[unlikely]] {
    ...
    // ── 2. Read buffer state once, clamp the write range ────────────────
    // A single post-coercion byteLength read. If valueOf/toString detached
    // the buffer, byteLength is 0 → offset >= end → we return without
    // touching typedVector (which would be nullptr). If valueOf shrank a
    // resizable ArrayBuffer, the clamp keeps us inside the new logical
    // length so we don't leak bytes past the resize boundary.
    size_t currentLength = castedThis->byteLength();
    if (offset > currentLength) offset = currentLength;
    if (end > currentLength) end = currentLength;
    if (offset >= end) {
        RELEASE_AND_RETURN(scope, JSValue::encode(castedThis));
    }

    // ── 3. Write. typedVector() is non-null here (byteLength > 0). ─────
    ...
    case ViewBranch: {
        auto* head = startPtr;
        size_t remain = span;
        size_t length = std::min<size_t>(viewValue->byteLength(), remain);
